...
// Make our parameters global, so parseArguments can set some
// of the fields
...
int CreateAThread(int i) {
	...
	return pthread_create(&thread, NULL, ecrash_test_thread,
					      (void *) parms);
} // CreateAThread
...
void BuildSymbolTable(eCrashSymbolTable *table)
{
	...
	qsort(symbols, table->numSymbols, sizeof(eCrashSymbol),
		  addressCompare);
...
int parseArguments(int argc, char *argv[])
{
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "cvqxn:s:t:h?",
...
int main (int argc, char *argv[])
{
    ...
    params.fd = open("eCrash.out.fd", O_WRONLY|O_CREAT,
