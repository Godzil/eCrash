...
typedef struct
{
    ...
    /*** Default signal to use to tell a thread to drop its stack. */
    int defaultBacktraceSignal;

    /*** How long to wait for a threads dump */
     ...
     * table, containing function names and start addresses. This table can be created using a script, or a
     * static table.
     *
     * If this variable is not null, it will be used, instead of backtrace_symbols, reguardless of the setting
     * of useBacktraceSymbols.
     */
    eCrashSymbolTable *symbolTable;

    /***
     * Array of crash signals to catch, ending in 0  I would have left it a [] array, but I do a static
     * copy, so I needed a set size.
     */
 ...
 * Initialize eCrash.
 * 
 * This function must be called before calling any other eCrash unctions. It sets up the global
 ...
 * UnInitialize eCrash.
 * 
 * This function may be called to de-activate eCrash, release the signal handlers, and free any
 * memory allocated by eCrash.
 *
 * @return Zero on success.
 */
 ...
 * 
 * This function must be called by any thread wanting it's stack dumped in the event of a crash.
 * The thread my specify what signal should be used, or the default, SIGUSR1 will be used.
 *
